...
export async function insertSubscriptionRecord(stripeSubscription: Stripe.Subscription){
    ...
    const {data: customer, error: custError} = await supabaseAdmin.from("billing_customers")
    .select("user_id").eq("id", subscription.customer_id )
    .limit(1)
    .single();
...
export async function verifyTrialEligibility(user_id: string, price_id: string): Promise<boolean> {
    ...
    const price = (await stripe.prices.retrieve(price_id, {expand: ["product"]}) as PriceWithProduct)
    if (!price){
    ...
    if (!subscription){
      console.log("free sub")
        return true
    }

    console.log("no sub for u")
    return false
}
