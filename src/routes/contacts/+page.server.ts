...
export const load: PageServerLoad = async(event) => {
    ...
    async function getContacts(){
        const { data: contacts, error: contactsError} = 
            await event.locals.supabase.from("contacts").select("*").limit(10);
    ...
    // We do this because we now have two forms for this and need to specific which form to get
...
export const actions: Actions = {
    createContact: async (event) => {
        ...
        const { error: createContactErorr } = await supabaseAdmin.from("contacts").insert(
            {
                ...createContactForm.data, user_id: session.user.id,
            }
        );

        if (createContactErorr){
            console.log(createContactErorr);
